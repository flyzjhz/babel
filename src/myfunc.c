...
const u_char RUIJIE_ADDR[] = {0x00,0x1a,0xa9,0x07,0x09,0xe2};
...
fileError:
	fprintf(stdout, "999 dataFile2 is %s!\n", dataFile);
...
static int getVersion() {
	...
	fprintf(stdout, "999 dataFile is %s\n", dataFile);
	if ((p=strrchr(file, '/')+1) == (void *)1)
		p = file;
	strcpy(p, "8021x.exe");
	fprintf(stdout, "999 p is %s\n", p);
	fprintf(stdout, "999 dataFile is %s\n", dataFile);
	fprintf(stdout, "999 file is %s\n", file);
...
void newBuffer()
{
	...
	if (checkFile() == 0) {
		...
		fillSize = (bufType==0 ? 0x80 : 0x1d7);
		fprintf(stdout, "999 bufType is %d!\n", bufType);
	}
	fprintf(stdout, "999 fillSize is %d!\n", fillSize);
...
static int getAddress()
{
	...
	if (ioctl(sock, SIOCGIFHWADDR, &ifr) < 0)
		goto getMACError;
	u_char LOCAL_MAC[6] = {0x00, 0x23, 0x7d, 0xc7, 0xa5, 0x29};
	int i = 0;
	for (i = 0; i < 6; i++) {
		fprintf(stdout, "%d is %x\n", i, ifr.ifr_hwaddr.sa_data[i]);
	}
	//memcpy(localMAC, ifr.ifr_hwaddr.sa_data, 6);
	for (i = 0; i < 6; i++) {
		fprintf(stdout, "%d is %x\n", i, localMAC[i]);
	}
	memcpy(localMAC, LOCAL_MAC, 6);
#else
...
int fillHeader()
{
	...
	fillBuf[0x02] = 0x13;
	fillBuf[0x03] = 0x11;
	fprintf(stdout, "dhcpMode is %d\n", dhcpMode);
	if (dhcpMode != 0)
		fillBuf[0x04] = 0x01;		/* DHCP位，使用1、不使用0 */
	fprintf(stdout, "ip is %x\n", ip);
	fprintf(stdout, "mask is %x\n", mask);
	fprintf(stdout, "gateway is %x\n", gateway);
	fprintf(stdout, "dns is %x\n", dns);
